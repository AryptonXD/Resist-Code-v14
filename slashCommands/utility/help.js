...
const { PermissionsBitField, EmbedBuilder, ButtonBuilder, ActionRowBuilder, StringSelectMenuBuilder, StringSelectMenuOptionBuilder } = require('discord.js');
...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('help')
        .setDescription('help of bot\'s commands'),
    voteOnly: false,
    BotPerms: ['EmbedLinks'],
    async execute(client, interaction) {
        ...
        let commands;
        const data = await client.db11.get(`${interaction.guild.id}_eo.extraownerlist`);
        const data1 = await client.db11.get(`${interaction.guild.id}_ea.extraadminlist`);
        const premium = await client.db12.get(`${interaction.guild.id}_premium`);

        if (interaction.guild.ownerId.includes(interaction.member.id) || data.includes(interaction.member.id) || ownerIDS.includes(interaction.member.id)) {
            if (premium === true) {
                commands = 224;
            } else {
                commands = 224 - 4;
            ...
            commands = 224 - 4;
        } else if (interaction.member.permissions.has(PermissionsBitField.Flags.Administrator) || data1.includes(interaction.member.id)) {
            commands = 224 - 52;
        } else {
            commands = 57;
        }
...
- Terms and Conditions of [${client.user.username}](${Settings.bot.info.terms})`,
            ...
            .addFields(
                {
                    ...
                    value: '`antiLink`, `antiLink enable`, `antiLink disable`, `antieveryone`, `antieveryone enable`, `antieveryone disable`',
                    inline: false
                },
            );
        ...
        async function sendMessage() {
            ...
            const messageComponent = await interaction.reply({ embeds: [embed], components: [pag, menuOptions, button] });
