...
async function handleOfficialCommand(client, message, args) {
  ...
  if (!officialRole) {
    ...
    const embed = new EmbedBuilder()
      .setColor(client.color)
      .setDescription(`${emoji.util.cross} | **Official Role** not found.`);
  ...
  if (hasOfficialRole) {
    ...
    embed.setDescription(`${emoji.util.tick} | Successfully removed <@&${officialRole}> from ${mentionedMember}`);
  } else {
    mentionedMember.roles.add(officialRole);
    embed.setDescription(`${emoji.util.tick} | Successfully added <@&${officialRole}> to ${mentionedMember}`);
  }
